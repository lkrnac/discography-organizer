...
public class ReferenceMediaNodeTest{
	...
	/**
	 * Prepares testing data for test {@link ReferenceMediaNodeTest#
	 * testCheckFullAlbumForSelection(int, String, StorageMetadataMaps, 
	 * BranchNodeStatus, MediaIssue, boolean, boolean)}
	...
	public Object [][] testCheckFullAlbumForSelection(){
		...
		MediaIssue mediaIssueMissing = new MediaIssue(TEST_PATH, 
				MediaIssueMessages.REFERENCE_FULL_MIRROR_MISSING, 
				relativePath, false);
		...
		MediaIssue mediaIssueIoError = new MediaIssue(TEST_PATH, 
				MediaIssueMessages.GENERIC_IO_ERROR + 
		...
		return new Object[][]{
			...
			//expect no issue
			new Object [] {i++, pathToFullMirror, getStorageMetadataMapsMock(pathToFullMirror, 1, true), 
					BranchNodeStatus.FOLDER, null, true, false},
			new Object [] {i++, pathToFullMirror, getStorageMetadataMapsMock(pathToFullMirror, 1, false), 
					BranchNodeStatus.FOLDER, null, true, false},
			...
			// - create various full album mirrors in storages meta-data maps and
			//expect error about various full mirrors found
			// - full mirror checks shouldn't affect the result
			new Object [] {i++, pathToFullMirror, getStorageMetadataMapsMock(pathToFullMirror, 2, false), 
					BranchNodeStatus.ERROR, mediaIssueVarious, true, false},
			new Object [] {i++, pathToFullMirror, getStorageMetadataMapsMock(pathToFullMirror, 2, true), 
					BranchNodeStatus.ERROR, mediaIssueVarious, true, false},
	 ...
	 * context initialization. All the instances needed from Spring context are mocked.
	 * <p>
	 * Verifies:
	 * <ul>
	 ...
	 * </ul>
	 * @param testCaseId identifies test case (not used)
	 * @param pathToFullMirror relative path to full mirror mock 
	 * @param storageMetadataMaps meta-data where are testing data for comparison stored  
	 * @param expectedStatus expected node status
	 * @param expectedIssue expected media issue for testing node 
	 * @param dirComparisonSucceed if the comparison of files in selection and 
	 * its full mirror should succeed 
	 * @param throwIoError if comparison of directories should throw IO error
	 * @throws DiscographyOrganizerException if I/O error occurs
	 */
	@Test(dataProvider = DP_TEST_CHECK_FULL_ALBUM_FOR_SELECTION)
	public void testCheckFullAlbumForSelection(int testCaseId, 
			String pathToFullMirror, StorageMetadataMaps storageMetadataMaps,
			BranchNodeStatus expectedStatus, MediaIssue expectedIssue, 
			boolean dirComparisonSucceed, boolean throwIoError) throws DiscographyOrganizerException{
		ReferenceMediaNode testingObjectSpy = initializeMocks(TEST_PATH, 
	...
	public Object [][] testCheckSelectionForFullAlbum(){
		...
		MediaIssue mediaIssueMissing = new MediaIssue(FULL_PARENT_ABSOLUTE_PATH, 
				MediaIssueMessages.REFERENCE_MISSING_SELECTION_MIRROR, 
				relativePath, false);
		...
		int i = 0;
		return new Object[][]{
			//create one selection album mirror in storages meta-data maps and
			//expect no issue
			...
			// - don't create mirror in storages meta-data maps
			//and expect warning and media issue creation
			// - comparison of directories doesn't have any sense and shouldn't affect the result 
	 ...
	 * @param dirComparisonSucceed if the comparison of files in selection and 
	 * its full mirror should succeed 
	 * @param expectedResult expected result of the check
	 * @param testCaseId identifies test case (not used)
	 * @param throwIoError if comparison of directories should throw IO error
	 * @throws DiscographyOrganizerException if IO error occurs 
	 */
	@Test(dataProvider = DP_TEST_CHECK_SELECTION_FOR_FULL_ALBUM)
	public void testCheckSelectionForFullAlbum(int testCaseId, 
			StorageMetadataMaps storageMetadataMaps,
			BranchNodeStatus expectedStatus, MediaIssue expectedIssue, 
			boolean dirComparisonSucceed, boolean expectedResult, 
			boolean throwIoError) throws DiscographyOrganizerException{
	 ...
	 * @param testingObjectSpy testing spy object
	 * @return created media issues list
	 */
	private MediaIssuesCollection createMediaIssuesMock(
			ReferenceMediaNode testingObjectSpy) {
	 ...
	 * @param parentDirectoryPath parent directory path of testing object 
	 * @param storageMetadataMaps Mock for storage meta-data maps (see {@link StorageMetadataMaps})
	 * @param dirComparisonSucceed desired result of directories comparison
	 * @param throwIoError if comparison of directories should throw IO error
	 * @return testing object spy
	 * @throws DiscographyOrganizerException declared to avoid compiler error (method is stubbed)
	 */
	private ReferenceMediaNode initializeMocks(String parentDirectoryPath,
		...
		if (throwIoError){
			Mockito.when(dirIoFacadeMock.compareDirectories(Mockito.any(File.class), 
					Mockito.any(File.class))).thenThrow(new DiscographyOrganizerException(
		...
		ReferenceMediaNode testingObject = 
				new ReferenceMediaNode(null, dirMock, parentDirectoryPath);
	...
	/**
	 * Creates and returns storage meta-data maps mock object with included
	 * mock {@link ReferenceMediaNode}, which suppose to be mirror album for testing
	 * <p>
	 * Is used for testing where methods where content in storage meta-data is expected
	 * @param pathToFileMock relative path to {@link ReferenceMediaNode} mock<br>
	 * This will be added into storage meta-data maps
	 * @param numberOfMirrors number of mirror mocks to be added for pathToFileMock
	 * @param passFullMirrorChecks if the checks for full mirror should pass
	...
	private StorageMetadataMaps getStorageMetadataMapsMock(String pathToFileMock, 
		...
		Mockito.when(storageMetadataMaps.toString()).thenReturn(
				"Mock for " + StorageMetadataMaps.class.getSimpleName() + 
				"[pathToFileMock = \"" + ((pathToFileMock == null) ? "null": pathToFileMock) + "\"]");
		
		Collection<ReferenceMediaNode> mirrorMocksList = 
				new ArrayList<ReferenceMediaNode>(numberOfMirrors);
		...
		if (pathToFileMock != null && numberOfMirrors > 0){
			for (int i = 0; i < numberOfMirrors; i++){
				...
				if (passFullMirrorChecks != null){
					Mockito.when(fullMirrorMock.checkSelectionForFullAlbum(
							Mockito.anyString())).thenReturn(passFullMirrorChecks);
				}
				mirrorMocksList.add(fullMirrorMock);
			}
		}
		
		Mockito.when(storageMetadataMaps.getReferenceItems(pathToFileMock))
			.thenReturn(mirrorMocksList);
		return storageMetadataMaps;
	}
	
	/**
	 * Check if testing object caused expected media issue
	 * @param expectedIssue media issue that is expected to be raised by testing method
	 * @param mediaIssuesList list where media issue should or shouldn't be stored
	 */
	private void verifyMediaIssue(MediaIssue expectedIssue, MediaIssuesCollection mediaIssuesList) {
		...
		if (expectedIssue == null) {
			Assert.assertEquals(iterator.hasNext(), 
					false, "No media issue is expected");
		} else {
			try{
