...
public class ReferenceMediaNode extends MediaBranchNode{
	 ...
	 * @param relativePath relative path within reference storage
	 */
	public ReferenceMediaNode(TreeStorageBranchNode parent, 
	...
	/**
	 * Setter
	 * @param inputMirror input mirror belonging to this reference media node
	 */
	...
	/**
	 * Setter
	 * @param fullMirror full album mirror
	 */
	...
	/**
	 * Setter
	 * @param selectionMirror selection album mirror
	 */
	 ...
	 * @param fullAlbum full album flag
	 */
	...
	/**
	 * Checks if selection mirror contains 
	 * hard links of this full album
	 * <p><b>
	 * This method expects that full mirror is initialized for this node 
	 * (see {@link ReferenceMediaNode#checkSelectionForFullAlbum(String)})
	 * </b>
	 */
		...
		if (getFullMirror() != null){
			try{
				if (!this.getDirectoryIoFacade().verifyHardLinks(getFullMirror().getFile())){
					this.getMediaIssuesList().add(new MediaIssue(this.getAbsolutePath(), 
							MediaIssueMessages.REFERENCE_NO_HARD_LINK_IN_SELECTION, 
				...
				getMediaIssuesList().add(new MediaIssue(this.getAbsolutePath(), 
						MediaIssueMessages.REFERENCE_HARD_LINK_IO_ERROR, 
						this.getRelativePath(), true));
				this.setNodeStatus(BranchNodeStatus.ERROR);
			}
		}
	 ...
	 * <p><b>
	 * It expects that whole storage was loaded into meta-data maps before
	 * </b>
	 * @param fullSubDirectory full sub-directory name
	...
	public boolean checkSelectionForFullAlbum(String fullSubDirectory) {
		...
		try{
			if (getSelectionMirror() == null) {
				...
				Collection<ReferenceMediaNode> selectionMirrors = 
						getStorageMetadataMaps().getReferenceItems(selectionPath);
		...
		} catch (DiscographyOrganizerException e){
			this.setNodeStatus(BranchNodeStatus.ERROR);
			getMediaIssuesList().add(new MediaIssue(
					this.getAbsolutePath(),
					e.getMessage(),
	 ...
	 * <p>
	 * It expects that whole storage was loaded into meta-data maps 
	 * (see {@link StorageMetadataMaps}) before
	 * @param fullSubDirectory full sub-directory name
	 */
	public void checkFullAlbumForSelection(String fullSubDirectory){
		...
		try {
			if (!this.isFullAlbum()) {
				...
				for (ReferenceMediaNode tmpMirror : 
					getStorageMetadataMaps().getReferenceItems(fullAlbumMirrorPath)){
